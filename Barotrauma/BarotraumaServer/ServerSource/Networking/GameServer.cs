...
namespace Barotrauma.Networking
{
    partial class GameServer : NetworkMember
    {
        ...
        private List<Client> oldPlayers = new List<Client>();
        private List<Client> readyMonsters = new List<Client>();
        private Random rnd = new Random();

        private IEnumerable<object> StartMonsters()
        {
            /*
               string[] xancrab = { "crawlercrablet", "helmetcrablet", "infector", "voidleech", "faradaycrab", "thermalcrab",
                   "skyholdercrab", "psychosiscrab", "nasonov" };

               string[] xanhusk = { "bomber", "huskslasher", "infector", "juggernaut", "paralyser",
                   "bruteflora", "mudraptorescort" };

               string[] xanthresher = { "xanothresher", "lionthresher", "mudraptorescort" };

               string[] xanfractal = { "fractalguardianre", "fractalguardianre2", "fractalguardianre3", "voidleech", "scavenger", 
                   "mudraptorescort" };

               string[] xansmall = { "crawlercrablet", "helmetcrablet", "huskhammerspawn", "infector", "lionthresher",
                   "fractalguardianre", "fractalguardianre2", "fractalguardianre3", "voidleech", 
                   "bomber", "huskslasher", "juggernaut", "paralyser", "bruteflora", "mudraptorescort" };

               string[] xantiny = { "crawlercrablet", "helmetcrablet", "infector", "voidleech", "bomber" };

               string[] xanrandos = { "bomber", "coelanthRe", "crawlercrablet", "helmetcrablet", "huskhammerspawn", "huskslasher",
                   "infector", "juggernaut", "paralyser", "faradaycrab", "thermalcrab", "skyholdercrab", "psychosiscrab", "nasonov",
                   "xanothresher", "hydrathresher", "lionthresher", "fractalguardianre", "fractalguardianre2", "fractalguardianre3",
                   "bruteflora", "immortal",  "voidleech", "voidstalker", "scavenger", "mudraptorescort", "bomber", "coelanthRe", "crawlercrablet", 
                   "helmetcrablet", "huskhammerspawn", "huskslasher", "infector", "juggernaut", "paralyser", "faradaycrab", "thermalcrab",
                   "skyholdercrab", "psychosiscrab", "nasonov", "xanothresher", "hydrathresher", "lionthresher", "fractalguardianre",
                   "fractalguardianre2", "fractalguardianre3", "bruteflora", "immortal", "voidleech", "voidstalker", "scavenger",
                   "mudraptorescort", "bossmoloch","huskbroodemperor", "huskbroodmother", "endwormre", "carrier", "carrierleviathan",
                   "observer", "charybdisre", "mudbouncer" };

                    
            string[][] monsters = { xanrandos, xantiny, xansmall, xanfractal, xanthresher, xanhusk, xancrab };

            string[] monsternames = { "random XAN monsters", "tiny XAN monsters", "small XAN monsters", "XAN fractal monsters", 
                "XAN thesher monsters", "XAN husk monsters", "XAN crab monsters" };

                                      */

            string[] randos = { "bonethresher", "coelanth", "crawler", "crawlerhusk", "fractalguardian",
                       "fractalguardian2", "hammerheadspawn", "humanhusk", "husk",
                       "legacycrawler", "legacyhusk", "leucocyte", "mantis", "molochbaby", "mudraptor",
                       "terminalcell", "tigerthresher", "watcher", "bonethresher", "coelanth", "crawler",
                       "crawlerhusk", "fractalguardian", "fractalguardian2", "hammerheadmatriarch", "hammerheadspawn",
                       "humanhusk", "husk", "legacycrawler", "legacyhusk", "leucocyte", "mantis",
                       "molochbaby", "mudraptor", "terminalcell", "tigerthresher", "watcher",
                       "legacymoloch", "moloch", "legacytigerthresher", "legacycharybdis", "charybdis", "hammerhead", "hammerheadgold",
                       "swarmcrawler"};

            string[] crawler = { "crawler", "crawlerhusk", "swarmcrawler" };

               string[] mudraptor = { "mudraptor", "legacycrawler" };

               string[] mantis = { "mantis", "legacycrawler" };

               string[] hammerheadspawn = { "hammerheadspawn" };

               string[] husk = { "humanhusk", "husk", "legacyhusk" };

               string[] smarthusk = { "humanhusk" };

               string[] terminal = { "terminalcell" };

               string[] cell = { "leucocyte", "terminalcell" };

               string[] small = { "crawler", "crawlerhusk", "swarmcrawler", "humanhusk", "husk",
                       "legacycrawler", "legacyhusk", "mantis", "mudraptor", "tigerthresher" };

               string[] aggressive = { "crawler", "crawlerhusk", "legacycrawler", "mantis", "mudraptor", "swarmcrawler" };

               string[] thresher = { "tigerthresher" };

               string[][] monsters = { randos, crawler, mudraptor, mantis, hammerheadspawn, husk, smarthusk, terminal, cell,
                       small, aggressive, thresher };

               string[] monsternames = { "random monsters", "crawlers", "mudraptors", "mantids", "hammerhead spawn",
                       "husk", "intelligent husk", "terminal cells", "cells", "small monsters", "aggressive monsters", "threshers" };


            int mapTop = (int)(Submarine.MainSub.WorldPosition.Y + Submarine.MainSub.Borders.Height / 2);
            int counter = 0;

            readyMonsters = new List<Client>();
            ...
            while (gameStarted && Level.Loaded.Type != LevelData.LevelType.Outpost)
            {
                ...
                if (counter == 120)
                {
                    ...
                    START:
                        ...
                        spawnpos.Y = spawnsub.WorldPosition.Y + (float)(2000 * rndy * signy);
                        ...
                        spawnpos.X = spawnsub.WorldPosition.X + (float)(2000 * rndx * signx);

                        float angleRadians = GetRadian(spawnpos, spawnsub.WorldPosition);

                        float rayy = (float)(250 * Math.Sin(angleRadians));
                        float rayx = (float)(250 * Math.Cos(angleRadians));
                        float rayoy = (float)(250 * Math.Sin(angleRadians + Math.PI / 2));
                        float rayox = (float)(250 * Math.Cos(angleRadians + Math.PI / 2));
                        ...
                        Vector2 raycsim = FarseerPhysics.ConvertUnits.ToSimUnits(rayc);
                        ...
                        if (rays < 20)
                        {
                            ...
                            rays += 1;
                            if (GameMain.World.RayCast(rayasim, raybsim).Any() || GameMain.World.RayCast(raycsim, raybsim).Any())
                            {
                                goto START;
                            }
                            gopos = spawnpos;
                        ...
                        string names = "";
                        float radius = Vector2.Distance(spawnpos, spawnsub.WorldPosition);
                        string dir = GetCard(GetAngle(angleRadians));

                        readyMonsters = new List<Client>(GameMain.Server.ConnectedClients.Where(c => (c.Character?.IsDead ?? true)
                        && !c.SpectateOnly && c.InGame));

                        while (readyMonsters.Any())
                        {
                            yield return new WaitForSeconds(1);
                            ...
                            SendDirectChatMessage("You hear something " + (int)radius + " away to the " + dir + " direction!",
            ...
            GameMain.Server.SendChatMessage("Click spectate in the lobby or type startspec; (don't forget the semicolon) in the " +
                "chatbox to join spectators if you don't want to spawn as a monster. Type stopspec; to be able " +
                "to respawn as a monster. Try help; to learn more.", ChatMessageType.Error);
        ...
        private bool Between(float A, float B, float Angle)
        {
            if (Angle < B && Angle > A)
            {
                return true;
        ...
        {
            if (Between(60, 120, angle))
            {
                return ("North");
            }
            if (Between(240, 300, angle))
            {
                return ("South");
            }
            if (Between(330, 360, angle) || Between(0, 30, angle))
            {
                return ("East");
            }
            if (Between(150, 210, angle))
            {
                return ("West");
            }


            if (Between(120, 150, angle))
            {
                return ("Northwest");
            }
            if (Between(30, 60, angle))
            {
                return ("Northeast");
            }

            if (Between(210, 240, angle))
            {
                return ("Southwest");
            }
            if (Between(300, 330, angle))
            {
                return ("Southeast");
            }
            ...
            if (type == null)
            {
                ...
                switch (command.ToLowerInvariant())
                {
                    ...
                    case "suicide":
                        ...
                        break;
                    case "findcoal":
                        string cdir = GetDirection(senderClient.Character.WorldPosition, Submarine.MainSubs[0].WorldPosition);
                        SendDirectChatMessage("Coalition submarine has been detected to the " + cdir + "!", senderClient, ChatMessageType.MessageBox);
                        break;
                    case "findsep":
                        if (Submarine.MainSubs.Length > 1)
                        {
                            string rdir = GetDirection(senderClient.Character.WorldPosition, Submarine.MainSubs[1].WorldPosition);
                            SendDirectChatMessage("Separatist submarine has been detected to the " + rdir + "!", senderClient, ChatMessageType.MessageBox);
                    ...
                    case "stopspec":
                        ...
                        break;
                    case "tip":
                        GameMain.Server.SendChatMessage("Type findsep; (don't forget the semicolon) to get the direction of the " +
                            "separatist sub. Type findcoal; to get the direction of the coalition sub. If you're lost, type suicide; " +
                            "in the chatbox to give up.", ChatMessageType.Error);
                        break;
                    case "help":
                        SendDirectChatMessage("suicide; -- Kills the players character. Use if lost or unable to play. \n" +
                            "findcoal; -- Finds the direction of the coalition submarine. \n" +
                            "findsep; -- Finds the direction of the separatist submarine. \n" +
                            "stopspec; -- Leaves spectator group and allows becoming a monster \n" +
                            "startspec; -- Joins spectator group and prevents becoming a monster", senderClient, ChatMessageType.Error);
                        break;
                    case "addplayer":
                        if (senderClient.HasPermission(ClientPermissions.Ban))
                        {
                            List<Client> newPlayers = new List<Client>(GameMain.Server.ConnectedClients.Where(c => !oldPlayers.Any(p => p == c)
                            && (c.Character?.IsDead ?? true) && !c.SpectateOnly && c.InGame));
                            if (newPlayers.Any())
                            {
                                foreach (Client c in newPlayers)
                                {
                                    Character bot = Character.CharacterList.Find(chr => chr.IsHuman && chr.IsBot && !chr.IsDead
                                    && Submarine.MainSubs.Any(s => Vector2.Distance(s.WorldPosition, chr.WorldPosition) < 2000));
                                    if (bot != null)
                                    {
                                        SetClientCharacter(c, bot);
                                        oldPlayers.Add(c);
                                        GameMain.Server.SendChatMessage(c.Name + "'s soul has entered " + bot.Name + "!", ChatMessageType.Error);
                                    }
                                    else { goto NOBOT; }
                                }
                                goto DONE;
                            }
                            SendDirectChatMessage("No players to add!", senderClient, ChatMessageType.MessageBox);
                            break;
                        NOBOT:
                            SendDirectChatMessage("Not enough bots!", senderClient, ChatMessageType.MessageBox);
                            break;
                        DONE:
                            SendDirectChatMessage("Finished adding players!", senderClient, ChatMessageType.MessageBox);
                            break;
                        } else
                        {
                            SendDirectChatMessage("Adding players failed!", senderClient, ChatMessageType.MessageBox);
                        }
                        break;
                    case "disarm":
                        if (senderClient.HasPermission(ClientPermissions.Ban))
                        {
                            string[] banned = { "tonicliquid", "steroids", "hyperzine",
                                "nucleardepthcharge", "nucleardepthdecoy", "nuclearshell" };
                            int count = 0;
                            ...
                            SendDirectChatMessage(count + " items removed!!!", senderClient, ChatMessageType.MessageBox);
                        }
                        break;
                    case "removeflare":
                        if (senderClient.HasPermission(ClientPermissions.Ban))
                    ...
                    case "addzombie":
                        if (senderClient.HasPermission(ClientPermissions.Ban))
                        {
                            List<Character> deadBots = new List<Character>(Character.CharacterList.Where(chr => chr.IsHuman && chr.IsDead
                            && Submarine.MainSubs.Any(s => Vector2.Distance(s.WorldPosition, chr.WorldPosition) < 2000)));
                            if (deadBots.Any())
                            {
                                foreach (Character chr in Character.CharacterList.Where(chr => chr.IsHuman && chr.IsDead))
                                {
                                    chr.Revive();
                                    Affliction zombie = new Affliction(AfflictionPrefab.List.First(a => a.AfflictionType == "huskinfection"), 100f);
                                    chr.CharacterHealth.ApplyAffliction(chr.AnimController.MainLimb, zombie);
                                }
                                goto DONE;
                            }
                            SendDirectChatMessage("No bots to turn into zombies!", senderClient, ChatMessageType.MessageBox);
                            break;
                        DONE:
                            SendDirectChatMessage("Finished adding zombies!", senderClient, ChatMessageType.MessageBox);
                            break;
                        }
                        break;
                    case "startvoip":
            ...
            foreach (Client client in ConnectedClients)
            {
                ...
                switch (type)
                {
                    ...
                    case ChatMessageType.Dead:
                        ...
                        if (client != senderClient && client.Character != null && !client.Character.IsDead && client.Character.IsHuman) continue;
                        break;
